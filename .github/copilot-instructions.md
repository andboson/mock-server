...
- Put reusable packages in `internal/`
...
- Use `gomock` (uber) and already generated mocks for testing.
- If no mocks are available, suggest generating them using `mockgen`.
- Always use `require` (from `github.com/stretchr/testify/require`) for checking errors in tests, instead of `assert`.
- Always run `make lint` and fix any issues before finalizing test code.
...
- place test data in a `testdata/` directory (`intrenal/testdata` or package specific `testdata/`), and load it in tests as needed
...
- Suggest using libraries like `github.com/go-playground/validator/v10` for struct validation
...
- Always run the linter (`make lint`) after checking tests and fix lint errors
